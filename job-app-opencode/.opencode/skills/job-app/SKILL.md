...
This is the OpenCode variant. It runs the same workflow as the single-context version but delegates the two review roles to subagents with their own sessions, so the checks are performed by something that did not write the draft and does not know why any line reads the way it does.

## Setup

Copying the `.opencode/` tree registers everything: the three subagents in `agents/`, the read-restriction plugin in `plugins/job-app.ts`, and this skill. Confirm the three agents are listed before relying on them.

The plugin runs inside OpenCode and has no external dependencies.
...
**Buys:** a genuinely fresh context. A subagent runs in its own child session, starting with its own system prompt and the delegation message, not the conversation history — so the verifier cannot be swayed by the reasoning that produced a line, because it never saw it. That is a real check rather than the same context grading its own work. It also keeps research output and page-by-page critique out of the main window.

**Partly buys:** each agent's `permission` frontmatter denies `edit`, `bash`, and every other tool it has no use for, so the verifier cannot edit what it is checking. That guarantee is real.

**Doesn't buy on its own:** limits on what a subagent may *read*. Telling the critic to look only at the PDF is a convention, not a constraint. `plugins/job-app.ts` closes this. It learns which agent owns each session from the `chat.params` hook, then checks every tool call in `tool.execute.before`: the main agent and any unrelated subagent pass through untouched, and the three roles above are limited to their own tools and a per-role read allowlist scoped to the active application.

Two things to know before relying on it:

- **It filters an argument rather than removing one.** A tool with no path parameter cannot be redirected at all; the plugin inspects the path the model chose and decides. The allowlist is structural, resolved against the active slug, and paths are canonicalized first so `..` traversal and symlinks resolve to their real target before matching. Reads are also confined to the directory OpenCode was started in.
- **It fails closed for the three roles and open for everyone else.** A missing `.active`, an unresolvable path, or a path outside the project blocks those three. The main session is never blocked by this plugin, because blocking it on a bad payload would break the session for a check that was never about it.

Be honest with the person about which guarantees are real.

Subagents also cannot ask the user anything — the `question` permission is denied to them. Every question to the candidate comes from the main session.

## How the main session runs

The main session drafts, orchestrates, and is the only thing that talks to the person. Delegate at three points, through the task tool:
...
**Write the active slug to `applications/.active` before delegating anything.** One line, the slug of the application being worked on. Update it when switching jobs. `plugins/job-app.ts` reads it to confine every subagent to that one application — with no `.active` file (and no `JOB_APP_SLUG` in the environment), every read is blocked and the subagents report they cannot reach anything.

No subagent can change `.active`, because none of them may edit or run commands. Only the main session can switch applications.
