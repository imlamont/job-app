...
**Releasing.** Users install from the `release` branch (`/plugin marketplace add imlamont/job-app@release`, or a clone of `release` for OpenCode), so changes on `main` reach nobody until they are pushed to `release`. A push to the `release` branch runs `.github/workflows/build-skill.yml`: it runs `build-skill.sh`, uploads the zip as a workflow artifact, and deletes and recreates the `job-app-skill` release with the zip attached, so the public download URL stays fixed. Bump `version` in `job-app-claudecode/.claude-plugin/plugin.json`. That is what decides whether already-installed Claude Code users receive changes. OpenCode and claude.ai installs are copies, so re-copying or re-uploading is the update.
